...
function trainSeating(penumpang) {
    ...
    for (let i = 0; i < penumpang.length; i++) {
        if (dataPenumpang[penumpang[i].gerbong] == undefined) {
            dataPenumpang[penumpang[i].gerbong] = []
        }
        var isiClass = {}
        isiClass.nama = penumpang[i].nama
        isiClass.seat = penumpang[i].seat
        dataPenumpang[penumpang[i].gerbong].push(isiClass)
    }
